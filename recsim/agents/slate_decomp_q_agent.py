...
def score_documents(user_obs,
  ...
  """
    scores = np.array([])
    for doc in doc_obs:
        # Changed to work with preference dynamics
        dominant_topic = np.zeros(shape=len(doc))
        dominant_topic[np.argmax(doc)] = 1
        doc_score = np.dot(user_obs, dominant_topic)
        scores = np.append(scores, doc_score)
...
class SlateDecompQAgent(dqn_agent.DQNAgentRecSim,
    ...
    """
        ...
        # Changed to work with preference dynamics
        expected_response_names = ['clicked', 'engagement']
        ...
        # Changed to work with preference dynamics
        self._click_response_index = response_names.index('clicked')
        self._reward_response_index = response_names.index('engagement')
        self._quality_response_index = response_names.index('quality')
        self._cluster_id_response_index = response_names.index('topic_id')
    ...
    def _build_select_slate_op(self):
        ...
        with tf.name_scope('select_slate'):
            self._output_slate = self._select_slate_fn(self._slate_size, p_no_click,
                                                       p, q)
        # Luca - Added comment because of printing to console
