...
class Runner(object):
  ...
  def _run_one_episode(self, phase=None):
...
class TrainRunner(Runner):
  ...
  def __init__(self, max_training_steps=250000, num_iterations=100,
    ...
    self._output_dir = os.path.normpath(os.path.join(self._base_dir, 'train'))
    self._checkpoint_dir = os.path.normpath(os.path.join(self._output_dir, 'checkpoints'))
  ...
  def run_experiment(self):
    ...
    # Finish the training run, flush to weights and biases
  ...
  def _run_train_phase(self, total_steps):
    ...
    while num_steps < self._max_training_steps:
      episode_length, _ = self._run_one_episode(phase='train')
...
class EvalRunner(Runner):
  ...
  def __init__(self,
    ...
    self._output_dir = os.path.join(self._base_dir, 'eval')
  ...
  def _run_eval_phase(self, total_steps):
    ...
    while num_episodes < self._max_eval_episodes:
      _, episode_reward = self._run_one_episode(phase='eval')
      episode_rewards.append(episode_reward)
      num_episodes += 1
      if num_episodes % 20 == 0:
        tf.logging.info('Completed {0} evaluation episodes.'.format(num_episodes))
